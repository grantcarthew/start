...
All UTD content files are processed through Go's `text/template` engine. Temp files are created **only for external files** (files outside the working directory, such as those from the CUE module cache). Files within cwd are validated for existence but not copied.

Temp directory location: `.start/temp/` (project-local)

File naming convention: `<type>-<path-segments>.md`

Examples (external files only):
...
Files within cwd (relative paths or absolute paths under working directory):
...
Conditional temp file creation (cwd vs external):

- Files within cwd are already accessible to AI agents - no copy needed
- Avoids unnecessary disk I/O for common case (files within cwd)
- External files (CUE cache) require copying to accessible location
- `{{.file}}` returns meaningful paths for files within cwd ("AGENTS.md" vs temp path)
- File existence is still validated for files within cwd (fails fast if missing)

## Trade-offs

Accept:

- Requires `.start/temp/` in `.gitignore` (only created when external files are used)
- Slightly more complex logic (cwd vs external check)
- User manages cleanup manually

Gain:

- Zero security configuration for agent access
- No disk I/O for files within cwd (common case)
- Meaningful `{{.file}}` paths for files within cwd
- External files still accessible via temp copies
...
- Pro: Skip I/O for files within cwd (already accessible)
- Pro: Meaningful `{{.file}}` paths for files within cwd
...
- **Files within cwd** (`file: "AGENTS.md"`): `{{.file}}` returns the original path for semantic clarity in prompts
- **Module files** (`file: "@module/task.md"`): `{{.file}}` returns the temp path because the CUE cache is inaccessible to AI agents

This ensures prompts like "Read {{.file}} for context" produce meaningful output ("Read AGENTS.md") for files within cwd while still providing accessible paths for module files.
...
The `{{.role_file}}` placeholder follows the same cwd vs external logic as UTD `{{.file}}`:

| Role Source | `{{.role_file}}` Value |
|-------------|------------------------|
| `file:` with cwd path | Original file path (already accessible) |
...
- No unnecessary temp files for role files within cwd
...
3. Determine if file is within cwd:
   - Relative path (e.g., `AGENTS.md`, `./file.md`) → within cwd
   - Absolute path under working directory → within cwd
   - Otherwise → external
4. For files within cwd: validate existence with `os.Stat()`
5. For external files: copy to `.start/temp/` and update path
6. Process through Go template engine
7. Provide path via `{{.file}}` or content via `{{.file_contents}}`

File naming derivation (for external files):
...
Creation: Only when external files are used (e.g., registry-installed assets with `@module/` paths). Projects using only files within cwd will never see this directory.
